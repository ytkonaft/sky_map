...
!(function() {
  ...
  Celestial.display = function(config) {
    ...
    prjMap = Celestial.projection(cfg.projection)
      .rotate(rotation)
    ...
    function load() {
          ...
          container
            ...
            .datum(
              d3.geo
                .circle()
                .angle([90])
                .origin(transformDeg(poles[key], euler[trans]))
            )
            .attr('class', key)
        }
      }
      ...
      d3.json(path + 'mw.json', function(error, json) {
        if (error) {
          window.alert(
            "Your Browser doesn't support local file loading or the file doesn't exist. See readme.md"
          )
    ...
    function reproject(config) {
      ...
      var prjTo = Celestial.projection(config.projection)
        .center(ctr)
        .translate([width / 2, width / prj.ratio / 2])
      ...
      d3.select({})
        ...
        .tween('projection', function() {
          ...
          prjMap = Celestial.projection(config.projection)
            .rotate(rot)
            .translate([width / 2, height / 2])
            .scale(scale * zoomlevel)
          map.projection(prjMap)
        ...
        if (cfg.constellations.names) {
          ...
          container.selectAll('.constname').each(function(d) {
            if (clip(d.geometry.coordinates)) {
              setConstStyle(
                d.properties.rank,
                cfg.constellations.namestyle.font
              )
              var pt = prjMap(d.geometry.coordinates)
      ...
      if (cfg.stars.show) {
        ...
        container.selectAll('.star').each(function(d) {
          if (
            ...
            if (
              cfg.stars.names &&
              d.properties.mag <= cfg.stars.namelimit * adapt
            ) {
      ...
      if (cfg.dsos.show) {
        container.selectAll('.dso').each(function(d) {
          if (
            ...
            if (
              cfg.dsos.names &&
              dsoDisplay(d.properties, cfg.dsos.namelimit)
            ) {
    ...
    function constName(d) {
      return cfg.constellations.desig
        ? d.properties.desig
        : d.properties[cfg.lang]
    }
  ...
  var settings = {
    ...
    constellations: {
      ...
      namestyle: {
        ...
        opacity: 0.8,
        font: [
          "14px 'Lucida Sans Unicode', Helvetica, Arial, sans-serif", // Different fonts for brighter &
...
*/
  ...
  function form(cfg) {
    ...
    col
      .append('label')
      .attr('for', 'stars-names')
      .html('Show designations')
    col
      .append('input')
      .attr('type', 'checkbox')
      .attr('id', 'stars-names')
      .property('checked', config.stars.names)
      .on('change', apply)

    col
      .append('label')
      .attr('for', 'stars-namelimit')
      .html('down to mag')
    col
      .append('input')
      .attr('type', 'number')
      .attr('id', 'stars-namelimit')
      .attr('title', 'Star designaton display limit (magnitude)')
      .attr('value', config.stars.namelimit)
      .attr('max', '6')
      .attr('min', '-1')
      .attr('step', '0.1')
      .on('change', apply)
    ...
    col
      .append('label')
      .attr('for', 'stars-desig')
      .attr('title', 'include HD/HIP designations')
      .html('all')
    col
      .append('input')
      .attr('type', 'checkbox')
      .attr('id', 'stars-desig')
      .property('checked', config.stars.desig)
      .on('change', apply)

    col
      .append('label')
      .attr('for', 'stars-proper')
      .html('proper names')
    col
      .append('input')
      .attr('type', 'checkbox')
      .attr('id', 'stars-proper')
      .property('checked', config.stars.proper)
      .on('change', apply)

    col
      .append('label')
      .attr('for', 'stars-propernamelimit')
      .html('down to mag')
    col
      .append('input')
      .attr('type', 'number')
      .attr('id', 'stars-propernamelimit')
      .attr('title', 'Star name display limit (magnitude)')
      .attr('value', config.stars.propernamelimit)
      .attr('max', '6')
      .attr('min', '-1')
      .attr('step', '0.1')
      .on('change', apply)
    ...
    col
      ...
      .attr('type', 'checkbox')
      .attr('id', 'dsos-names')
      .property('checked', config.dsos.names)
      .on('change', apply)

    col
      .append('label')
      .attr('for', 'dsos-desig')
      .html('or designations')
    col
      .append('input')
      .attr('type', 'checkbox')
      .attr('id', 'dsos-desig')
      .property('checked', config.dsos.desig)
      .on('change', apply)
    ...
    col
      .append('label')
      .attr('for', 'constellations-names')
      .html('Show names')
    col
      .append('input')
      .attr('type', 'checkbox')
      .attr('id', 'constellations-names')
      .property('checked', config.constellations.names)
      .on('change', apply)

    col
      .append('label')
      .attr('for', 'constellations-desig')
      .html('abbreviated')
    col
      .append('input')
      .attr('type', 'checkbox')
      .attr('id', 'constellations-desig')
      .property('checked', config.constellations.desig)
      .on('change', apply)

    col
      .append('label')
      .attr('for', 'constellations-lines')
      .html('with lines')
    col
      .append('input')
      .attr('type', 'checkbox')
    ...
    col
      .append('label')
      .attr('title', 'Laitudet/longitude grid lines')
  ...
  function geo(cfg) {
    ...
    if (has(cfg, 'geopos') && cfg.geopos !== null && cfg.geopos.length === 2)
      geopos = cfg.geopos
    var col = frm
      .append('div')
      .attr('class', 'col')
    ...
    col
      ...
      .attr('value', dateFormat(date, zone))
    ...
    col
      ...
      .property('checked', cfg.horizon.show)
      .on('change', go)
    col
      .append('label')
    ...
    col
      ...
      .property('checked', cfg.planets.show)
      .on('change', go)
  ...
  var Kepler = function() {
    ...
    function equatorial(pos) {
      ε =
        (23.439292 -
          0.0130042 * dat.cy -
          1.667e-7 * dat.cy * dat.cy +
          5.028e-7 * dat.cy * dat.cy * dat.cy) *
        deg2rad
      sinε = Math.sin(ε)
      cosε = Math.cos(ε)
      var o =
        id === 'lun' ? { x: 0, y: 0, z: 0 } : { x: pos.x, y: pos.y, z: pos.z }
      dat.xeq = dat.x - o.x
      ...
      dat.dec = Math.atan2(
        dat.zeq,
        Math.sqrt(dat.xeq * dat.xeq + dat.yeq * dat.yeq)
      )
      if (id === 'lun') dat = moon_corr(dat, pos)
      dat.pos = [dat.ra / deg2rad, dat.dec / deg2rad]
      dat.rt = Math.sqrt(
        dat.xeq * dat.xeq + dat.yeq * dat.yeq + dat.zeq * dat.zeq
      )
    }
    ...
    function cartesian() {
      ...
      dat.x =
        dat.r *
        (Math.cos(dat.N) * Math.cos(u) -
          Math.sin(dat.N) * Math.sin(u) * Math.cos(dat.i))
      dat.y =
        dat.r *
        (Math.sin(dat.N) * Math.cos(u) +
          Math.cos(dat.N) * Math.sin(u) * Math.cos(dat.i))
  ...
  var datetimepicker = function(cfg, callback) {
    ...
    function daySel() {
      ...
      for (var i = 0; i < 7; i++) {
        cal
          .append('div')
          .classed({ date: true, weekday: true })
          .html(days[i])
      }
      for (i = 0; i < 42; i++) {
